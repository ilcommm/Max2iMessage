...
1. Скачайте **Max2iMessage-v1.3-macos.zip** из [Releases](https://github.com/ilcommm/Max2iMessage/releases/latest)
...
**Совет для общего Mac:** в **Настройки → Пересылка в iMessage** выберите «Только уведомление» — на этом компьютере в Messages не будет сохраняться текст чужих переписок, на iPhone получателя уведомление всё равно придёт.
...
| Messages не всплывает | iMessage отправляется в фоне, без активации приложения Messages |

При **запуске из Xcode** (Debug-сборка) режим можно включать и выключать в **Настройки → Приложение → Режим приватности** — для отладки.

## Формат пересылки в iMessage

В **Настройки → Пересылка в iMessage** для каждого аккаунта:

- **Полный текст** — `Имя: текст сообщения` (по умолчанию, удобно если пользуетесь сами)
- **Только уведомление** — `Имя написал(а) в MAX` — текст сообщения не попадает в iMessage
